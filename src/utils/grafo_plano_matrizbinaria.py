...
for image_path in image_paths:
    ...
    # Matriz M^2 (conexões de caminho 2)
    matriz_quadrada = np.dot(adjacency, adjacency)
    df_quad = pd.DataFrame(matriz_quadrada, columns=rotulos, index=rotulos)
    out_csv2 = OUT_DIR / f"{image_path.stem}_matriz_binaria_quadrado.csv"
    df_quad.to_csv(out_csv2)
    print(f"Matriz binária ao quadrado salva: {out_csv2}")
